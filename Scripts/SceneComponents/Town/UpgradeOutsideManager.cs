...
class RoofDataCollection {
    public readonly Vector3[] offsetPosY = new Vector3[12] {
        new Vector3(0f, -22f, -5f), new Vector3(0f, -22f, -5f), new Vector3(0f, -19.5f, -5f),new Vector3(-3f, -7f, -5f),
		new Vector3(-2f, -19.5f, -5f), new Vector3(0, -22f, -5f) , new Vector3(0, -22f, -5f), new Vector3(0, -22f, -5f),
		new Vector3(3.5f, -22f, -5f), new Vector3(0, -26f, -5f) , new Vector3(0, -10f, -5f), new Vector3(0, -28f, -5f),
    };

	public readonly string[] NameSpecify = new string[12] {
		"roof_0001", "roof_0002", "roof_0003", "roof_0004", 
		"roof_0005", "roof_0006", "roof_0007", "roof_0008",
		"roof_0009", "roof_0010", "roof_0011", "roof_0012",
	};

	public readonly  int[] upgradePrices = new int[12] {
		1000, 1200, 1300, 1500, 
		1750, 2200, 2500, 3000,
		3000, 3000, 3000, 3000,
	};
...
class AwningDataCollection {
	...
	public readonly string[] NameSpecify = new string[9] {
		"awning_0001", "awning_0002", "awning_0003", "awning_0004", "awning_0005", 
        "awning_0006", "awning_0007", "awning_0008", "awning_0009",
	};

	public readonly int[] upgradePrices = new int[9] {
		1000, 1500, 2000, 2800, 3200,
        3200, 3200, 3200, 3200,
	} ;
}
...
class AccessoriesDatacollecction {
	public const int Number_Of_AccessorriesCollections = 28;
	...
	public readonly string[] NameSpeccify = new string[Number_Of_AccessorriesCollections] {
		...
		"access_0015", "access_0016", "access_0017", "access_0018", "access_0019", "access_0020", "access_0021", 
		"access_0022", "access_0023", "access_0024", "access_0025", "access_0026", "access_0027", "access_0028", 
	};
...
public class UpgradeOutsideManager : MonoBehaviour
{
	...
	private const int NumberOfRoofsPage = 2;
	private const int NumberOfAwningPage = 2;
	private const int NumberOfTablePage = 3;
	private const int NumberOfAccessoriePage = 4;
	int amountPages;
	int currentPage;
	...
	public void ActiveRoof ()
	{
		for (int i = 0; i < 7; i++) {
			...
			if(UpgradeOutsideManager.CanDecorateRoof_list.Contains(i) == false) {
				...
				itemPrice_textmesh[i].text = roofData.upgradePrices[i].ToString();
	...
	public void ActiveAwning() {		
		for (int i = 0; i < 7; i++) {
			...
			if(UpgradeOutsideManager.CanDecorateAwning_list.Contains(i) == false) {
				itemPrice_textmesh[i].gameObject.SetActiveRecursively(true);
				itemPrice_textmesh[i].text = awningData.upgradePrices[i].ToString();
				itemPrice_textmesh[i].Commit();
			}
			else {
				itemPrice_textmesh[i].gameObject.SetActiveRecursively(false);
...
//		nextButton_Obj.active = false;
	...
	public void HaveNextPageCommand() {
		if (currentStateBehavior == StateBehavior.activeRoof)
			amountPages = NumberOfRoofsPage;
		else if(currentStateBehavior == StateBehavior.activeAwning)
			amountPages = NumberOfAwningPage;
		else if(currentStateBehavior == StateBehavior.activeTable)
	...
	void GoToPage (int pageSpecify)
	{
		if (currentStateBehavior == StateBehavior.activeRoof) {
			for(int i = 0; i < 7; i++) 
			{
				...
				if(j < roofData.NameSpecify.Length) {
					/// Display item sprite.
					int spriteID = upgrade_sprites [i].GetSpriteIdByName(roofData.NameSpecify [j]);
					upgrade_sprites [i].spriteId = spriteID;
					//@-- Display price.
					if(UpgradeOutsideManager.CanDecorateRoof_list.Contains(j) == false) {
						itemPrice_textmesh[i].gameObject.SetActiveRecursively(true);
						itemPrice_textmesh[i].text = roofData.upgradePrices[j].ToString();
						itemPrice_textmesh[i].Commit();
					}
					else {
						itemPrice_textmesh[i].gameObject.SetActiveRecursively(false);
					...
					/// Display item sprite.
					int id = upgrade_sprites [i].GetSpriteIdByName("none_button_up");
					upgrade_sprites[i].spriteId = id;
					itemPrice_textmesh[i].gameObject.SetActiveRecursively(false);                    
				}
			}
		}
            ...
            {
                int j = i + (7 * pageSpecify);
                if (j < awningData.NameSpecify.Length) {
                    /// Display item sprite.
                    int spriteID = upgrade_sprites[i].GetSpriteIdByName(awningData.NameSpecify[j]);
                    upgrade_sprites[i].spriteId = spriteID;
                    //@-- Display price.
                    if (UpgradeOutsideManager.CanDecorateAwning_list.Contains(j) == false)
                    {
                        itemPrice_textmesh[i].gameObject.SetActiveRecursively(true);
                        itemPrice_textmesh[i].text = awningData.upgradePrices[j].ToString();
                        itemPrice_textmesh[i].Commit();
                    }
                    else
                    {
    ...
    {
        if(currentStateBehavior == StateBehavior.activeRoof || currentStateBehavior == StateBehavior.activeAwning 
		   || currentStateBehavior == StateBehavior.activeTable || currentStateBehavior == StateBehavior.activeAccessories)
        {
			int id = 0;
            switch (blockName)
            {
    ...
    private void CheckingCanBuyItem(int targetItem_id)
    {
        if (currentStateBehavior == StateBehavior.activeRoof)
		{
			if(targetItem_id < roofData.NameSpecify.Length)
			{
				if(CanDecorateRoof_list.Contains(targetItem_id) == false)
				{
		            if (Mz_StorageManage.AccountBalance >= roofData.upgradePrices[targetItem_id])
					{
			...
			if(targetItem_id < awningData.NameSpecify.Length)
			{
				if(CanDecorateAwning_list.Contains(targetItem_id) == false) {
					if(Mz_StorageManage.AccountBalance >= awningData.upgradePrices[targetItem_id]) {					
						/// Todo... Asking to buy item.
						confirmWindow_Obj.SetActiveRecursively(true);
						this.PlaySoundOpenComfirmationWindow();
						transaction_id = targetItem_id;
	...
	/// This Func call form StageManager.
	/// When user input to confirm transactions.
	/// </summary>
    internal void UserConfirmTransaction()
    {
        if (currentStateBehavior == StateBehavior.activeRoof)
        {
            ...
            Mz_StorageManage.AccountBalance -= roofData.upgradePrices[transaction_id];
			this.ReFreshAccountBalanceTextDisplay();
        }
		else if(currentStateBehavior == StateBehavior.activeAwning) {
			...
			//@!-- Deductions AvailableMoney and Redraw GUI identity.
			confirmWindow_Obj.SetActiveRecursively(false);
			Mz_StorageManage.AccountBalance -= awningData.upgradePrices[transaction_id];
			this.ReFreshAccountBalanceTextDisplay();
		}
		else if(currentStateBehavior == StateBehavior.activeTable) {
